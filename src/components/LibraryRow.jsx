    ...
    >
      <div className="flex items-center gap-6">
        ...
        <div className="flex flex-col gap-1">
          <h3 className="font-editorial text-[16px] md:text-[18px] text-[#DDDDDD] group-hover:text-white transition-colors">
            {title}
          </h3>
          <span className="font-geist text-xs text-[#666666] group-hover:text-[#888888] transition-colors">
            by {author}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col items-end gap-2">
        {/* Status Badge */}
        <div
          className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[10px] uppercase font-bold tracking-wider ${getStatusStyle()}`}
        >
          {getStatusIcon()}
          <span>{status}</span>
        </div>
